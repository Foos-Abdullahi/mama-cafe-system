...
import { ArrowLeft, Save, Edit2, Search } from 'lucide-react';
...
export default function DailyWaitressesEdit({
    ...
    const [assignments, setAssignments] = useState<WaitressAssignment[]>(() => {
        ...
        return waitresses.map((w) => {
            ...
            return {
                waitress_id: w.id,
                name: w.name,
                assigned_number: saved ? saved.assigned_number : '',
    ...
    const allPoolNumbers = Array.from(
        new Set([
            ...cafeFixedNumbers,
            ...assignments.map((a) => a.assigned_number).filter(Boolean),
        ]),
