...
export function AppSidebar() {
    ...
    const user = (props.auth as any)?.user as
    ...
    const filteredOverview = overview.filter((item) => {
        if (item.title === 'Dashboard') {
            return (
                role === 'admin' || role === 'manager' || role === 'operations'
            );
        }
    ...
    return (
        <Sidebar collapsible="icon" variant="inset">
            ...
            <SidebarContent className="[scrollbar-width:none] space-y-1 overflow-x-hidden overflow-y-auto px-2 py-2 [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden [&::-webkit-scrollbar]:h-0 [&::-webkit-scrollbar]:w-0">
                {/* Overview */}
                {filteredOverview.length > 0 && (
                    <SidebarGroup className="px-0 py-0">
                        <SidebarGroupLabel className="px-2.5 text-xs font-semibold tracking-wider text-sidebar-foreground/60 uppercase">
                            Overview
                        </SidebarGroupLabel>
                ...
                {filteredSections.map((section) => {
                    const isSectionHasActiveChild = section.children.some(
                        ...
                        >
                            <SidebarGroup className="px-0 py-0">
                                ...
                                >
                                    <CollapsibleTrigger
                                        className={cn(
                                            'flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-xs font-semibold tracking-wider uppercase transition-all duration-200 hover:bg-sidebar-accent/40 hover:text-sidebar-foreground',
                                ...
                                <CollapsibleContent className="data-[state=closed]:animate-collapse-up data-[state=open]:animate-collapse-down overflow-hidden transition-all">
                                    <SidebarMenu className="mt-1 ml-2 space-y-0.5 border-l border-sidebar-border/40 pl-2 group-data-[collapsible=icon]:mt-0 group-data-[collapsible=icon]:ml-0 group-data-[collapsible=icon]:space-y-1 group-data-[collapsible=icon]:border-l-0 group-data-[collapsible=icon]:pl-0">
